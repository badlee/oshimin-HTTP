...
exports.start = function(settings) {
	process.title = "Oshimin Labs - HTTP";
	...
	if(globalSettings.https.key!="" && globalSettings.https.cert!=""){
		var serverSecure = https.createServer(globalSettings.https,serv);
	}
	...
	if(globalSettings.path.plugin != undefined){
		fs.stat(globalSettings.path.plugin, function (err, stats) {
			...
			if(globalSettings.https.key!="" && globalSettings.https.cert!=""){
				multi.listen({
					port: globalSettings.https.port,
					nodes: globalSettings.nodes
				}, server);
			}
			*/
			...
			if(globalSettings.https.key!="" && globalSettings.https.cert!=""){
				serverSecure.listen(globalSettings.https.port);
			};
...
function handleRequest(req,res,cleanPathname,newSessionId){
	...
	req.fileInfo = t;
	delete t;
	log.info(moduleName,"Request");
	log.info(moduleName,sys.inspect([path,req.direct,req.url]));
	//fix index path
	var c = path.split("/");
	if(c[c.length-1] == ""){
		//define index.path
...
function serverSideRunning(newfileName,request,response,file,lastMod,sessionId,isForceEval){
	...
	var executeTheSSJFile = function (errRead, functoRun) {
		try{
			...
			//============================================
			//Genrating Context
			var MaxtimeToExec =  setTimeout(function(){
				while(currentAsemaphore.p()>0);
			},(globalSettings.server_script.max_wait != 0) ? globalSettings.server_script.max_wait*1000 : 3*60*1000);
			var context = {
...
function sendHeaders(req, res,responseHead, length, content_type, modified_time,sessionId) {
	...
	responseHead.headers["Server"] = globalSettings.web_app_name+"/"+globalSettings.web_app_version+", Node.js/"+process.version;
...
function defaultSettings(settings){
	...
	settings.web_app_version = app_version;
	...
	if(settings.https.port == undefined)
		settings.https.port = 443;
	
	if(settings.https.key == undefined)
		settings.https.key = "";

	settings.https.key = appProtocol(settings.https.key ,settings.__dir);
	var e;
	if(settings.https.key != ""){
		try{
			settings.https.key = fs.readFileSync(settings.https.key);
		}catch(e){
			settings.https.key = "";
		}
	}
	if(settings.https.cert == undefined)
		settings.https.cert = "";
	
	settings.https.cert = appProtocol(settings.https.cert ,settings.__dir);
	
	if(settings.https.cert != ""){
		try{
			settings.https.cert = fs.readFileSync(settings.https.cert);
		}catch(e){
			settings.https.cert = "";
		}
	}
...
process.on('uncaughtException', function (err) {
  log.err(moduleName,'Caught exception :'+sys.inspect(err.stack));
  globalContext.alfred.close(new Function());
});
process.on('SIGINT', function () {
  console.log("");
  log.warn(moduleName,'Got SIGINT.  Press Control-D to exit.');
  process.exit();
});
